...
The _"university"_ `Document` is encoded to a `class` named _UniversityRecord_.
...
* Open the link:{examplesdir}/src/UserProfileDemo.Repositories/DatabaseManager.cs[DatabaseManager.cs] file and locate the `GetDatabaseAsync` method. The prebuilt database is common to all users of the app (on the device). So it will be loaded once and shared by all users on the device.
+
[source,c#]
----
...
* In the **DatabaseManager.cs** file, locate the `CreateUniversitiesDatabaseIndexes` method. We create an index on the `name` and `location` properties of the documents in the _university_ database.
+
[source,c#]
----
...
* In the link:{examplesdir}/src/UserProfileDemo.Repositories/BaseRepository.cs[BaseRepository.cs] file, locate the `Dispose` function. 
+
[source,c#]
----
...
* Open the link:{examplesdir}/src/UserProfileDemo.Repositories/UniversityRepository.cs[UniversityRepository.cs] file and locate the `SearchByName` method.
+
[source,c#]
----
