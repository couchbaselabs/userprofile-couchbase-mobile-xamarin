...
* *CouchbaseLabs.MVVM*: A .NET Standard project consisting of navigation, depedency injection, and MVVM helpers classes agnostic to platform dependencies. 

* *CouchbaseLabs.MVVM.Forms*: A .NET Standard project consisting of navigation, depedency injection, and MVVM helpers classes specifically supporting Xamarin.Forms. 
...
[source,swift]
----
include::{examplesdir}/UserProfileDemo.Models/University.cs[tags=universityrecord]
----
...
* In the `UserProfileDemo.iOS` project, locate the `universities.cblite2` folder
+
image::cblite_location.png[Prebuilt Database Location]

==== Android

* In the `UserProfileDemo.Android` cfrtg project, locate the `universities.zip` file. Note that the cblite folder will be extracted from the zip file.
+
image::cblite_location.png[Prebuilt Database Location]

==== UWP

* In the `UserProfileDemo.UWP` project, locate the `universities.cblite2` folder
+
image::cblite_location.png[Prebuilt Database Location]

=== Loading the Prebuilt Database
* Open the **DatabaseManager.cs** file and locate the `GetDatabaseAsync()` function. The prebuilt database is common to all users of the app (on the device). So it will be loaded once and shared by all users on the device.
+
[source,c#]
----
include::{examplesdir}/UserProfileDemo.Repositories/DatabaseManager.cs[tags=openPrebuiltDatabase]
----
...
[source,c#]
----
include::{examplesdir}/UserProfileDemo.Repositories/DatabaseManager.cs[tags=prebuiltdbconfig]
----

* Then we determine if the "universities" database already exists at the specified location. It would not be present if this is the first time we are using the app, in which case, we locate the _"universities.cblite"_ resource within the platfor project and we copy it over to the Database folder.
+
If the database is already present at the specified Database location, we simply open the database.
+ 
[source,c#]
----
include::{examplesdir}/UserProfileDemo.Repositories/DatabaseManager.cs[tags=prebuiltdbopen]
----
...
[source,c#]
----
include::{examplesdir}/UserProfileDemo.Repositories/DatabaseManager.cs[tags=createUniversityDatabaseIndexes]
----

=== Closing the Database
When a user logs out, we close the pre-built database along with other user-specific databases

* In the **UniversityRepository.cs** file, locate the `closePrebuiltDatabase()` function. 
+
[source,c#]
----
...
[source,c#]
----
include::{examplesdir}/UserProfileDemo.Repositories/UniversityRepository.cs[tags=fetchUniversityRecords]
----
...
[source,c#]
----
include::{examplesdir}/UserProfileDemo.Repository/UniversityRepository.cs[tags=buildquery]
----
+
...
[source,c#]
----
include::{examplesdir}/UserProfileDemo.Repositories/UniversityRepository.cs[tags=runquery]
----
<1> Create an instance of <<University>> type
<1> Use specific type getters to fetch property values. The <<University>> instance is populated with these property values.
